...
function mapKey(k) {
  ...
  if (/saturated fat|fatty acid,?\s*total\s*saturated/.test(s)) return "sat_fat_g";
...
async function resolveDetailUrl(item) {
  ...
  const modules = [item.module, "myfcd97", "myfcdindustri", "myfcdcurrent"]
    .filter(Boolean);
...
async function fetchDetail(url, nameFromList) {
  ...
  $("table").each((_, table) => {
    ...
    const nameCol  = headers.findIndex((h) => /^nutrient$/.test(h)) >= 0
    ...
    $(table).find("tbody tr").each((_, tr) => {
      ...
      const k = $(cells[nameCol] || cells[0]).text().trim();
      const v = $(cells[valueCol] || cells[cells.length - 1]).text().trim();
      if (k) kv[k] = v;
    });
  ...
  const id = nameFromList.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "");
  const rec = {
    id,
    name: nameFromList,
    aliases: [],
    category: guessCategory(nameFromList),
    default_portion: isDrink ? { unit: "cup", ml: 300 } : { unit: "plate", grams: 400 },
  };
...
(async () => {
  ...
  for (const item of rows) {
    ...
    if (!url) {
      console.warn(`[skip] cannot resolve detail URL | id=${item.id} module=${item.module} name="${item.name}"`);
      continue;
    }
  ...
  const outPath = path.join(__dirname, "..", "ml_service", "data", "curated", "_myfcd_batch.json");
