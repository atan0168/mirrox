...
export class MockHealthProvider implements HealthProvider {
  ...
  async getDailyHRVMs(start: Date, end: Date): Promise<number | null> {
    const seed = parseInt(`${start.getFullYear()}${start.getMonth()}${start.getDate()}`, 10) + 7;
    return Math.round((50 + seededRandom(seed) * 50) * 10) / 10; // 50-100 ms
  }

  async getDailyRestingHeartRateBpm(start: Date, end: Date): Promise<number | null> {
    const seed = parseInt(`${start.getFullYear()}${start.getMonth()}${start.getDate()}`, 10) + 13;
  ...
  async getDailyActiveEnergyKcal(start: Date, end: Date): Promise<number | null> {
    const seed = parseInt(`${start.getFullYear()}${start.getMonth()}${start.getDate()}`, 10) + 23;
  ...
  async getDailyMindfulMinutes(start: Date, end: Date): Promise<number | null> {
    const seed = parseInt(`${start.getFullYear()}${start.getMonth()}${start.getDate()}`, 10) + 31;
    return Math.round(seededRandom(seed) * 30); // 0-30 min
  }

  async getDailyRespiratoryRateBrpm(start: Date, end: Date): Promise<number | null> {
    const seed = parseInt(`${start.getFullYear()}${start.getMonth()}${start.getDate()}`, 10) + 37;
  ...
  async getDailyWorkoutsCount(start: Date, end: Date): Promise<number | null> {
    const seed = parseInt(`${start.getFullYear()}${start.getMonth()}${start.getDate()}`, 10) + 41;
