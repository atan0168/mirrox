...
export default function ThisMealCard() {
  ...
  const onAdd = async () => {
    ...
    const kcal    = Number.isFinite(kcalNum as number) ? (kcalNum as number) : undefined;
            ...
            <View key={it.id} style={styles.row}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.itemSub}>
                  {it.energy_kcal != null ? `${Math.round(it.energy_kcal)} kcal` : 'kcal N/A'}  •  x{it.qty}
                </Text>
              </View>
              <Pressable onPress={() => removeItem(it.id)} style={styles.deleteBtn}>
      ...
      >
        ...
        <View style={styles.modal}>
          ...
          <View style={styles.modalActions}>
            ...
            >
              <Text style={{ color: '#fff', opacity: !name.trim() ? 0.6 : 1 }}>Add</Text>
            </Pressable>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  deleteBtn:   { paddingHorizontal:10, paddingVertical:6, borderRadius:10, borderWidth:1, borderColor:'#FCA5A5', backgroundColor:'#FEF2F2' },
  deleteTxt:   { color:'#B91C1C', fontWeight:'700' },

  ghostBtn:    { paddingVertical:8, paddingHorizontal:12, borderRadius:10, borderWidth:1, borderColor:'#E5E7EB', backgroundColor:'#fff' },
  ...
  modal:       { position:'absolute', left:18, right:18, top:'25%', backgroundColor:'#fff', borderRadius:16, padding:16, borderWidth:1, borderColor:'rgba(17,24,39,0.08)' },
  modalTitle:  { fontWeight:'700', fontSize:16, marginBottom:8 },
  input:       { borderWidth:1, borderColor:'#E5E7EB', borderRadius:10, padding:10, marginBottom:10, backgroundColor:'#fff' },
  ...
  pillPrimary: { paddingHorizontal:12, paddingVertical:8, borderRadius:999, backgroundColor:'#2563EB' },
