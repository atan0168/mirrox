...
export function SmogController({ 
  ...
  const smokeTexture = useLoader(THREE.TextureLoader, require('../../../assets/smoke-default.png')) as THREE.Texture;
  ...
  const particleGeometry = useMemo(() => getDefaultParticleGeometryGenerator(), []);
  const particleMaterial = useMemo(() => getDefaultParticleMaterialGenerator(), []);
    ...
    if (enabled) {
      ...
      if (groupRef.current.children.length === 0) {
        ...
        for (let p = 0; p < particles.length; p++) {
          ...
          particle.material = particleMaterial(p, [smokeTexture], { opacity: opacity * intensity, density, color });
          ...
          const height = minBounds[1] + Math.random() * (maxBounds[1] - minBounds[1]);
          
          const x = Math.cos(angle) * radius + (Math.random() - 0.5) * 2;
          const y = height;
          const z = Math.sin(angle) * radius + (Math.random() - 0.5) * 2 - layerOffset;
  ...
  }, [enabled, particles, scene, particleGeometry, particleMaterial, maxBounds, minBounds, enableTurbulence, smokeTexture, size, density, camera]);
  ...
  useFrame((state, delta) => {
    ...
    particlesRef.current.forEach((particle, index) => {
      ...
      particle.position.add(
        tempVec3.set(velocity.x, velocity.y, velocity.z).multiplyScalar(delta * 20)
      );
      ...
      const distanceFromCenter = particle.position.distanceTo(new THREE.Vector3(0, 0, 0));
      ...
      if (distanceFromCenter > fadeStart) {
        const fadeRatio = Math.max(0, 1 - (distanceFromCenter - fadeStart) / (maxDistance - fadeStart));
