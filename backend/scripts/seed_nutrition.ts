...
function J(x: any) { return x == null ? null : JSON.stringify(x); }
...
function sanitizeNutrients(n: any) {
  if (!n) return null;
  const out: any = {};
  for (const k of ["energy_kcal","carb_g","sugar_g","fat_g","sat_fat_g","protein_g","fiber_g","sodium_mg"]) {
    const v = n[k];
    out[k] =
  ...
  const apply = (list: Food[]) => {
    for (const r of list) {
      ...
      if (!prev) {
        ...
        if (!merged.default_portion && r.default_portion) merged.default_portion = r.default_portion;
        ...
        const KEYS = ["energy_kcal","carb_g","sugar_g","fat_g","sat_fat_g","protein_g","fiber_g","sodium_mg"] as const;
        for (const key of KEYS) if (n100g[key] == null && n2[key] != null) n100g[key] = n2[key];
        for (const key of KEYS) if (n100ml[key] == null && n2ml[key] != null) n100ml[key] = n2ml[key];
...
(function main() {
  ...
  `);
  const insFts = db.prepare(`INSERT INTO foods_fts (rowid, name, aliases) VALUES (?, ?, ?)`);
  ...
  console.log(`✅ Seed done. CURATED: ${curated.length}, MyFCD: ${myfcd.length}, merged: ${merged.length}`);
})();
