...
router.post('/user-dict', (req, res) => {
  const { user_id, phrase, canonical_food_id, canonical_food_name } = req.body || {};
  if (!user_id || !phrase) return res.status(400).json({ ok:false, error:'user_id & phrase required' });
  const now = Date.now();
  db.prepare(`
  ...
  `).run(user_id, String(phrase).trim().toLowerCase(), canonical_food_id ?? null, canonical_food_name ?? null, now, now);
  res.json({ ok:true });
});
...
router.get('/user-dict', (req, res) => {
  ...
  const rows = db.prepare(`SELECT * FROM user_dict WHERE user_id=? ORDER BY updated_at DESC`).all(user_id);
  res.json({ ok:true, data: rows });
});

/** 2.3 Insert meal event (for habit learning) */
router.post('/meal-event', (req, res) => {
  const { user_id, ts, food_id, food_name, portion_json, source } = req.body || {};
  if (!user_id || !food_name) return res.status(400).json({ ok:false, error:'user_id & food_name required' });
...
router.get('/predict', (req, res) => {
  ...
  const byFood = new Map<string, { name:string; score:number; lastTs:number; count:number }>();
  for (const r of sameSlot) {
    const key = r.food_id ?? r.food_name;
    const prev = byFood.get(key) ?? { name: r.food_name, score: 0, lastTs: 0, count: 0 };
  ...
  const arr = [...byFood.entries()].map(([k,v]) => ({ key:k, name:v.name, score:v.score, lastTs:v.lastTs, count:v.count }))
  ...
  let ask = false;
  if (top && top.p >= 0.6) {
    const recentTwo = sameSlot.filter(r => (r.food_id ?? r.food_name) === top.key).slice(0, 2);
    ask = recentTwo.length >= 1;
  }

  res.json({
    ok: true,
    now, dow, hour: hr, slot,
...
router.get('/predictive-candidate', (req, res) => {
    ...
    const hourParam = req.query.hour !== undefined ? Number(req.query.hour) : null;
    const hour = (hourParam != null && !Number.isNaN(hourParam)) ? hourParam : new Date().getHours();
    const days = req.query.days ? Math.max(1, Math.min(90, Number(req.query.days))) : 28;
    ...
    const food = db.prepare(`SELECT name FROM foods WHERE id=?`).get(food_id) as { name?: string } | undefined;
